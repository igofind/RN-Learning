...
export default class MaskList extends PureComponent {
    ...
    render() {
        ...
        return (
            <View ref={(e) => this._userList = e} style={[theme.maskContainer, this.state.isExpand ? {} : styles.hide]}>
                <Animated.View style={ theme.mask }/>
                <View style={styles.list}>
                    <TouchableOpacity activeOpacity={1} onPress={() => this.changeUser(Config.CONS, true)}>
                        <View style={styles.listItem}>
                            <Text
                                style={[styles.itemText, this._checkUser(Config.CONS) && styles.itemActive]}>{Config.CONS_TITLE}</Text>
                    ...
                    <TouchableOpacity activeOpacity={1} onPress={() => this.changeUser(Config.COMP, false)}>
                        <View style={styles.listItem}>
                            <Text
                                style={[styles.itemText, this._checkUser(Config.COMP) && styles.itemActive]}>{Config.COMP_TITLE}</Text>
